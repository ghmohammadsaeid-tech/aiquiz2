...
[{ "q": "سوال", "o": ["گزینه۱", "گزینه۲", "گزینه۳", "گزینه۴"], "a": 0, "c": "دسته" }]`;
  ...
  const handleManualProcess = () => {
    try {
      ...
      const jsonStr = startIndex !== -1 && endIndex !== -1 ? cleanJson.substring(startIndex, endIndex + 1) : cleanJson;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-fade-in text-right">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] shadow-sm border-2 dark:border-slate-700">
        <label className="text-[10px] font-black text-slate-400 block mb-4 uppercase tracking-widest">انتخاب موتور هوشمند (AI Logic)</label>
      ...
      <div className={`rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden transition-all duration-500 ${isPremium ? 'bg-slate-900 border border-amber-500/30' : 'bg-indigo-600'}`}>
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-right">
            <h2 className="text-2xl font-black mb-1">طراح {selectedEngine} 🤖</h2>
          ...
          <div className="flex bg-black/20 backdrop-blur-xl p-1.5 rounded-2xl border border-white/10 w-full md:w-auto">
            <button onClick={() => setMethod('manual')} className={`flex-1 md:px-6 py-2.5 rounded-xl font-bold transition-all text-xs ${method === 'manual' ? 'bg-white text-indigo-600 shadow-xl' : 'text-white'}`}>روش دستی</button>
            <button onClick={() => setMethod('direct')} className={`flex-1 md:px-6 py-2.5 rounded-xl font-bold transition-all text-xs ${method === 'direct' ? 'bg-white text-indigo-600 shadow-xl' : 'text-white'}`}>تولید مستقیم</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <div className="dark:bg-slate-800 bg-white p-6 rounded-[2rem] border-2 dark:border-slate-700 shadow-sm space-y-5">
            <h3 className="font-black text-sm dark:text-white border-b-2 dark:border-slate-700 pb-3">تنظیمات محتوا</h3>
            <div>
              <label className="text-[10px] font-black text-slate-400 block mb-2">موضوع آزمون</label>
              <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="مثلاً: فیزیک کوانتوم" className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl outline-none font-bold text-slate-900 dark:text-white focus:border-indigo-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-black text-slate-400 block mb-2">تعداد</label>
                <select value={count} onChange={(e) => setCount(parseInt(e.target.value))} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl font-black text-slate-900 dark:text-white"><option value={5}>۵</option><option value={10}>۱۰</option><option value={20}>۲۰</option></select>
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-400 block mb-2">سطح</label>
                <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl font-black text-slate-900 dark:text-white"><option value="آسان">آسان</option><option value="متوسط">متوسط</option><option value="سخت">سخت</option></select>
              </div>
            </div>
          </div>
        </div>
          ...
          {method === 'direct' ? (
            <div className="dark:bg-slate-800 bg-white p-8 rounded-[2rem] border-2 dark:border-slate-700 shadow-sm h-full flex flex-col justify-center text-center">
              <div className="mb-8">
                ...
                <h3 className="text-xl font-black dark:text-white">طراحی مستقیم با {selectedEngine}</h3>
                <p className="text-slate-500 text-xs mt-3 px-6">با کلیک روی دکمه زیر، جمی‌نای با منطق {selectedEngine} شروع به تولید سوالات می‌کند.</p>
              </div>
            ...
            <div className="dark:bg-slate-800 bg-white p-6 rounded-[2rem] border-2 dark:border-slate-700 shadow-sm space-y-6">
              <div>
                <label className="text-[10px] font-black text-slate-400 block mb-3 uppercase">۱. کپی پرامپت برای {selectedEngine}:</label>
                <div className="relative">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(editablePrompt); const engine = ENGINES.find(e => e.name === selectedEngine); if(engine) window.open(engine.url, '_blank'); }} className="absolute bottom-4 left-4 p-2 bg-indigo-600 text-white rounded-lg text-[10px] font-bold shadow-lg">کپی و بازکردن {selectedEngine}</button>
                </div>
              </div>
              <div className="pt-4 border-t-2 dark:border-slate-700">
                <label className="text-[10px] font-black text-slate-400 block mb-3 uppercase">۲. خروجی را اینجا قرار دهید:</label>
                <textarea value={manualJson} onChange={(e) => setManualJson(e.target.value)} placeholder="Paste JSON here..." className="w-full h-28 p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl outline-none text-xs font-mono text-slate-900 dark:text-white" />
                <button onClick={handleManualProcess} className="w-full mt-4 py-4 bg-emerald-600 text-white rounded-2xl font-black text-sm shadow-lg active:scale-95 transition-all">استخراج سوالات</button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-[2rem] border-2 border-emerald-100 dark:border-emerald-800 gap-4">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              ...
              <button onClick={() => {
                ...
                setPreview([]);
                alert('سوالات در بانک ذخیره شدند.');
              }} className="flex-[2] px-6 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black shadow-lg">افزودن به بانک سوالات</button>
            </div>
          </div>
        </div>
      )}
